...
/**
 * Frequency copy of Mekanism's inventory frequency that records {@link TileEntityNuclearEntangloporter} participants while
 * accepting every stock entangloporter payload plus radioactive chemicals.
 */
public class NuclearInventoryFrequency extends Frequency implements IMekanismInventory, IMekanismFluidHandler,
      IMekanismStrictEnergyHandler, ITileHeatHandler, IChemicalTracker {

    @SuppressWarnings("removal")
    public static final Codec<NuclearInventoryFrequency> CODEC = RecordCodecBuilder.create(instance -> instance.group(
        ...
        NuclearInventoryFrequency frequency = new NuclearInventoryFrequency(name, owner.orElse(null), securityMode);
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, NuclearInventoryFrequency> STREAM_CODEC = StreamCodec.composite(
          Frequency.baseStreamCodec(NuclearInventoryFrequency::new), Function.identity(),
    ...
    private final Table<ResourceKey<Level>, BlockPos, TileEntityNuclearEntangloporter> activeEntangloporters = Tables.newCustomTable(new IdentityHashMap<>(), TreeMap::new);
    ...
    public NuclearInventoryFrequency(String name, @Nullable UUID owner, SecurityMode securityMode) {
        super(NuclearFrequencyTypes.NUCLEAR_INVENTORY, name, owner, securityMode);
        presetVariables();
    }

    private NuclearInventoryFrequency(String name, @Nullable UUID owner, String ownerName, SecurityMode securityMode) {
        super(NuclearFrequencyTypes.NUCLEAR_INVENTORY, name, owner, ownerName, securityMode);
        presetVariables();
    }
    ...
    private void presetVariables() {
        ...
        // Mekanism's stock validator rejects radioactive attributes; this is the sole intentional behavior difference.
        chemicalTanks = Collections.singletonList(storedChemical = BasicChemicalTank.createAllValid(MekanismConfig.general.entangloporterChemicalBuffer.get(), this));
        inventorySlots = Collections.singletonList(storedItem = EntangloporterInventorySlot.create(this));
        energyContainers = Collections.singletonList(storedEnergy = BasicEnergyContainer.create(
              MekanismConfig.general.entangloporterEnergyBuffer.getAsLong(),
              this
        ));
        heatCapacitors = Collections.singletonList(storedHeat = BasicHeatCapacitor.create(
              HeatAPI.DEFAULT_HEAT_CAPACITY,
    ...
    public boolean update(BlockEntity tile) {
        ...
        if (tile instanceof TileEntityNuclearEntangloporter entangloporter) {
            // Mekanism marks freshly deserialized frequencies as invalid until a live tile
            // validates them. Flip the switch here so the nuclear variant exposes its tanks
            // immediately after being selected just like the stock quantum entangloporter.
            if (!isValid()) {
                setValid(true);
                changedData = true;
            }
            activeEntangloporters.put(tile.getLevel().dimension(), entangloporter.getBlockPos(), entangloporter);
        } else {
            activeEntangloporters.remove(tile.getLevel().dimension(), tile.getBlockPos());
        }
        return changedData;
    }
    ...
    public void handleEject(long gameTime) {
        if (isValid() && !activeEntangloporters.isEmpty() && lastEject != gameTime) {
            lastEject = gameTime;
            ...
            int expected = 6 * activeEntangloporters.size();
            ...
            if (!typesToEject.isEmpty()) {
                for (TileEntityNuclearEntangloporter entangloporter : activeEntangloporters.values()) {
                    if (!entangloporter.canFunction()) {
                        continue;
                    }
                    ServerLevel level = (ServerLevel) entangloporter.getLevel();
                    if (level == null || !level.shouldTickBlocksAt(ChunkPos.asLong(entangloporter.getBlockPos()))) {
                        continue;
                    }
                    Direction facing = entangloporter.getDirection();
                    for (Map.Entry<TransmissionType, Consumer<?>> entry : typesToEject.entrySet()) {
                        TransmissionType transmissionType = entry.getKey();
                        ConfigInfo config = entangloporter.getConfig().getConfig(transmissionType);
                        if (config != null && entangloporter.getEjector().isEjecting(config, transmissionType)) {
                            for (Map.Entry<RelativeSide, DataType> sideEntry : config.getSideConfig()) {
                                if (sideEntry.getValue().canOutput()) {
                                    ...
                                    accept(entry.getValue(), entangloporter, side, transmissionType);
                                }
                            }
                        }
                    }
    ...
    private static <TYPE> void accept(Consumer<TYPE> consumer, TileEntityNuclearEntangloporter entangloporter, Direction side, TransmissionType transmissionType) {
        TYPE cachedCapability = entangloporter.getCachedCapability(side, transmissionType);
    ...
    private static class SendingEnergyAcceptorTarget extends EnergyAcceptorTarget implements Runnable, Consumer<IStrictEnergyHandler> {
        ...
        SendingEnergyAcceptorTarget(int expectedSize, IEnergyContainer storedEnergy, long toSend) {
            super(expectedSize);
    ...
    private static class SendingFluidHandlerTarget extends FluidHandlerTarget implements Runnable, Consumer<IFluidHandler> {
        ...
        SendingFluidHandlerTarget(@NotNull FluidStack toSend, int expectedSize, IExtendedFluidTank storedFluid) {
            super(expectedSize);
            this.toSend = toSend;
    ...
    private static class SendingChemicalHandlerTarget extends ChemicalHandlerTarget implements Runnable, Consumer<IChemicalHandler> {
        ...
        SendingChemicalHandlerTarget(ChemicalStack toSend, int expectedSize, IChemicalTank storedChemical) {
            super(expectedSize);
            this.toSend = toSend;
